...
typedef struct {
  IrType type;
  int t, t1, t2, label;//t為臨時的目標參數, t1和t2為臨時參數, label為標記
  char *s, *op;//*s為目標參數, *op為operation運算子
} IR; //將以上定義為一個名為IR的陣列結構


extern IR ir[];// 宣告一個結構為IR、名稱為ir的陣列
extern int irTop;//記錄此陣列的大小
extern int irPass2();
